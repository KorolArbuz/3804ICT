...
def add_engineered_features(frame, blocks):
    ...
    if "A" in blocks:
        ...
        values["ratio_mean_bill_to_limit"] = safe_divide(np.nanmean(bills, axis=1), limit)
        values["ratio_mean_payment_to_limit"] = safe_divide(np.nanmean(payments, axis=1), limit)
        values["ratio_bill_change_to_limit"] = safe_divide(bills[:, 0] - bills[:, 5], limit)
        values["limit_denominator_valid"] = (np.isfinite(limit) & (limit > 0)).astype(float)
    if "B" in blocks:
