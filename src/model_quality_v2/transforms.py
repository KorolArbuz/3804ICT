...
from .features import (
    BLOCK_A_COLUMNS, BLOCK_B_COLUMNS, MONEY_COLUMNS, NOMINAL_COLUMNS,
    OTHER_NUMERIC_COLUMNS, PAY_STATUS_COLUMNS, add_engineered_features, signed_log,
)
...
class StructuredPayTransformer(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        self.categories_ = [
            np.sort(np.unique(values[np.isfinite(values[:, index]) & (values[:, index] <= 0), index]))
...
def _money_pipeline(name):
    ...
    if name == "standard":
        ...
        steps.extend((("signed_log", SignedLogTransformer()), ("scaler", StandardScaler())))
    elif name == "yeo_johnson":
        steps.append(("scaler", PowerTransformer(method="yeo-johnson", standardize=True)))
    elif name == "robust":
...
class V2Preprocessor:
    ...
    def fit(self, frame, y=None):
        ...
        transformers = [
            ("money", _money_pipeline(self.configuration.money_transform), list(MONEY_COLUMNS)),
            ("other", _scaled_pipeline(StandardScaler()), list(OTHER_NUMERIC_COLUMNS)),
        ]
        if self.configuration.pay_representation == "raw":
            transformers.append(("pay", _scaled_pipeline(StandardScaler()), list(PAY_STATUS_COLUMNS)))
        else:
            transformers.append(("pay_struct", StructuredPayTransformer(), list(PAY_STATUS_COLUMNS)))
        ...
        if "A" in self.configuration.feature_blocks:
            transformers.append(("block_a", _scaled_pipeline(StandardScaler()), list(BLOCK_A_COLUMNS)))
        if "B" in self.configuration.feature_blocks:
            transformers.append(("block_b", _scaled_pipeline(StandardScaler()), list(BLOCK_B_COLUMNS)))
        ...
        self.feature_names_out_ = np.asarray(self.transformer_.get_feature_names_out(), dtype=str)
    ...
    def transform(self, frame):
        if not hasattr(self, "transformer_"):
            raise ValueError("fit must be called before transform")
        values = np.asarray(self.transformer_.transform(self._prepare(frame)), dtype=np.float64)
        values[:, self.pay_group_indices_] *= math.sqrt(self.configuration.pay_group_weight)
